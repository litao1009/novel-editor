...
`.opencode/commands/` 目录存放命令定义文件（`.md` 格式），opencode TUI 自动扫描并注册为 `/` 命令。

### 文件规范

每个命令文件遵循统一结构：
- **YAML front matter**：定义命令元信息（`name` / `description` / `subtask`）
- **Markdown 正文**：即 prompt 模板，`{input}` 会被替换为用户在 `/` 命令后输入的参数

### 创建新命令

1. 在 `.opencode/commands/` 下创建 `{命令名}.md`
2. 文件头添加 YAML front matter：
   ```yaml
   ---
   name: 命令名
   description: 命令描述
   ...
   ---
   ```
3. 正文编写 prompt 模板，定义命令的执行流程、参数、输出格式
4. 保存后在 TUI 中通过 `/{命令名}` 调用

### 现有命令一览

| 命令 | 说明 | 语法 |
|------|------|------|
| `zwf_train_origin` | 训练基准小说白皮书 | `/zwf_train_origin batch` 批量扫描 |
| | | `/zwf_train_origin {原作名}` 指定训练 |
...
| 2026-07-01 | `zwf_train_origin` | **新增命令系统**：创建 `.opencode/commands/zwf_train_origin.md`，将基准小说训练提取为独立命令。支持批量扫描和指定训练，源文件名不再固定（支持任意 .txt/.md 及多文件列表），白皮书输出名强制固定 base_whitepaper.md。`global_manager` Task 1 精简为委派指令（20行→8行），ReadMe 新增"命令系统"章节 | 全局调度、命令体系、基准训练流程 |
